...
abstract: >
  ...
  WORCS principles are implemented in the R package \pkg{worcs}, which offers an
  RStudio 
  ...
  \pkg{worcs} in \proglang{R}. All source code for the \proglang{R} package,
...
computational reproducibility for scholars working in \proglang{R} on a private computer,
conducting research that involves academic prose, analysis code,
and (optionally) tabular data - but it is flexible enough to accommodate many
other scenarios. The workflow (Figure \@ref(fig:workflow)) 
constitutes a step-by-step procedure that researchers can follow to make a 
research project (including data, code, and a report) open and reproducible.
...
been implemented for \proglang{R} users [@rcoreteamLanguageEnvironmentStatistical2020].
The \proglang{R} package \pkg{worcs} [@worcspackage2020] offers a project template for RStudio
...
it meets best practices for open science, and illustrates the use of the \pkg{worcs}
package.

```{r workflow, fig.cap="Schematic illustration of the WORCS procedure.", out.width="100%"}
#knitr::include_graphics("workflow_graph/workflow.png")
...
The TOP-guidelines are one of the most influential operationalisations
...
Although there is a slight learning curve to transitioning to DDG, we strongly
believe that the investment will pay off for users of the workflow, as it did for us.
...
website or blog post. Moreover, the fact that code can be run each time the
document is compiled encourages researchers to work reproducibly,
...
In the \proglang{R} implementation of WORCS, we recommend centering a research project
around one dynamically generated \code{RMarkdown} document, which includes all 
analysis code. Longer scripts can be
stored in \code{.R} files, and called from the main document using the \code{source()}
function. When a reader or reviewer compiles this document, all code is run
automatically, thus verifying computational replicability. The \code{RMarkdown} 
document can be automatically formatted in many styles, including
APA style [thanks to the \proglang{R} package \pkg{papaja}, @austPapajaPrepareReproducible2020],
a host of other scientific formats 
[see @allaireRticlesArticleFormats2020], and as plain \code{Markdown}.
...
A \code{.gitignore} file lists files that should not be version-controlled.
...
remote repository service, GitHub, two notable alternatives exist: 
...
paper.
As of
version 0.1.2, \pkg{worcs} has been tested with these three remote repository services,
but has been optimized for use with GitHub.
A [vignette](https://cjvanlissa.github.io/worcs/articles/git_cloud.html) is included
with instructions for using alternative platforms. 
When initializing a new project using the \pkg{worcs} project template,
a 'https' remote repository address will usually work best with the authentication procedure outlined in the
[setup vignette](https://cjvanlissa.github.io/worcs/articles/setup.html).

The \proglang{R} implementation of WORCS uses the package \code{gert}, a Git client for \proglang{R}
[@oomsGertSimpleGit2019], to connect a local project to a remote repository.
The \pkg{worcs} package also contains a user-friendly function to set
the default user credentials, \code{git_user()}, and a function to add, commit, and
push all changed files to a remote repository in one step, \code{git_update()}.
...
program is susceptible to changes, updates, and bugfixes. A lot of Open Source software,
...
This is effective, and also preserves software outside the \proglang{R} environment (e.g.,
preprocessing tools for neuroimaging data). A Docker-based workflow
...
Peikert and Brandmaier can be difficult to set up for novice users. 
The accompanying \proglang{R} package \pkg{repro}
...
data are stored.
The necessity to upload data for analysis is a risk with any
cloud-computing solution when human participants are involved.

For analyses conducted entirely in \proglang{R}, the recently released package \pkg{renv}
...
lightweight.
The text-based log file can be version controlled with Git. \pkg{renv}
...
project, \pkg{renv} will install all of the required packages onto a cache on their
computer. 
...
reproduce it in the future. For projects in \proglang{R}, \pkg{renv} meets these requirements.
...
Binary files, such as Word (\code{.docx}) or SPSS (\code{.sav}, \code{.spo}) files,
must be decoded first, and many require commercial software to do so.
The version control software Git is also designed for use with text-based files.
The Git change log reflects changes to text-based files on a line-by-line basis.
Binary files can be version controlled with Git, but 
the change log for a binary file are uninterpretable.
...
First, Git tracks line-by-line changes to text-based files,
based on line breaks in the text.
When writing a version-controlled paper in plain-text,
it is therefore recommended to start each sentence on a new line.
...
Second, the remote repository service GitHub rewards the use of text-based files by
rendering certain text-based filetypes for online viewing: 
For example, \code{.md}
(Markdown) files are displayed as web pages, and \code{.csv} files as spreadsheets.
The \pkg{worcs} implementation for \proglang{R} makes use of this fact by, for example,
rendering a codebook for the data as \code{.md} so visitors of a Git remote repository
can view it as a web page.

# Introducing the workflow 

We provide a conceptual outline of the WORCS procedure below, based on WORCS
Version 0.1.2. 
For users of the \pkg{worcs} \proglang{R} package, we highly recommend referring to
...
The \proglang{R} package \pkg{worcs} automates most steps of the workflow
for \proglang{R} users.
Several arguments support the choice to implement this workflow in \proglang{R}.
First, \proglang{R} and all of it extensions are free open source software, which
make it a tool of choice for open science.
Second, all tools required for an open science workflow are implemented in \proglang{R},
most of these tools are directly accessible through the user interface of RStudio,
and some of them are actively developed by the team behind RStudio.
Third, \proglang{R} is the second-most cited statistical software package [@muenchenPopularityDataScience2012], following SPSS, which has no support for any of the open science tools discussed in this paper.
Fourth, \proglang{R} is well-supported;
a vibrant and inclusive online community exists for \proglang{R},
developing new \proglang{R} methods and packages,
and providing support and tutorials for existing ones.
Finally, \proglang{R} is highly interoperable:
...
Moreover, wrappers are available for many tools developed in other programming languages, and code written in other programming languages can be evaluated from \proglang{R}, including \proglang{C++}, 
\proglang{Fortran}, and \proglang{Python} [@allaireMarkdownPythonEngine2020]. There are excellent free
resources for learning \proglang{R} [e.g., @grolemundDataScience2017].

Working with \proglang{R} is simplified immensely by using the RStudio integrated
development engine (IDE) for \proglang{R} [@rstudioteamRStudioIntegratedDevelopment2015].
...
by future users. The \pkg{worcs} \proglang{R} package installs a new RStudio project template.
...
Before you can use the \proglang{R} implementation of the WORCS workflow, you have to install the required software.
You only have to perform this setup procedure only once for each computer you
intend to use \pkg{worcs} on. It should take about 30 minutes, and it is documented
in [the setup vignette](https://cjvanlissa.github.io/worcs/articles/setup.html).
The setup vignette does not assume a prior installation of \proglang{R}, so it is suitable for novice users.
After setting up your system, you can use \pkg{worcs} for all of your
projects. The \proglang{R} implementation of the workflow introduced earlier is detailed,
step-by-step, in
...
for example, in \proglang{R}, package references can be found by
calling \code{citation("packagename")}. This returns an APA-style reference, and a
...
The \pkg{worcs} package includes a [vignette on citation](https://cjvanlissa.github.io/worcs/articles/citing.html) to explain the
fundamentals for novice users, and offer recommendations specific to \pkg{worcs}. We
...
The \pkg{worcs} package also offers original solutions for comprehensive citation.
Firstly,
...
Markdown uses the at-symbol (\code{@}) to cite a reference. \pkg{worcs} additionally 
reserves the "double at"-symbol (\code{@}) to cite a non-essential reference.
Users can render the manuscript either with, or without, comprehensive citations
by adapting the \code{knit} command in the front matter, setting it to
\code{knit: worcs::cite_all} to render all citations, and to 
\code{knit: worcs::cite_essential} to remove all *non-essential* citations.

With regard to the citation of \proglang{R} packages, it is worth noting that the \pkg{papaja} 
package [@austPapajaPrepareReproducible2020], which provides the \code{APA6}
template used in \pkg{worcs}, also
includes functions to automatically cite all packages used in the \proglang{R} session, and
...
\pkg{worcs} package offers a simple but flexible function to generate synthetic data,
\code{synthetic()}, which by default uses random forests to generate a model for
synthesis (see @wrightRangerFastImplementation2015). For data synthesis to work
properly, it is essential that the \code{class} (data type) of variables is defined
correctly. As of \pkg{worcs} version \code{r worcs:::cran_version()}, numeric, integer, factor, and logical data
are supported out of the box. Other types of variables should be converted to
one of these types, or users can use a custom \code{model_expression} when calling
\code{synthetic}.

### Processing data in WORCS

When initializing a new \pkg{worcs} project, an empty \proglang{R} script called \code{prepare_data.R} is generated.
As soon as raw data are collected, researchers should use this file to document
all steps necessary to load the 
data into \proglang{R}, pseudonimize it, and prepare it for analysis.
...
\code{-99} whereas they were coded as \code{-9} in all other variables, then we would recommend adding
...
The \pkg{worcs} package offers two functions for version controlling a record of the
...
In \pkg{worcs}, researchers can call the function \code{open_data(data)} to make the data
publicly available. This function stores the \code{data} object (such as a \code{data.frame} or
\code{matrix}) in a tabular data file (called \code{data.csv} by default).
...
This can be done through the Git panel in RStudio, or by running the \pkg{worcs}
function \code{git_update("commit message")}.
...
call \code{closed_data(data)}. This function also stores the data in a local \code{.csv} file
and generates a codebook,
but the original data file is added to \code{.gitignore} so it cannot be
accidentally added to the Git repository.
The function also computes a checksum for the original data, and logs it in the
\code{.worcs} project file. This checksum serves as the indelible record of the state 
of the raw data.
The function \code{closed_data()} also creates a synthetic dataset (called
\code{synthetic_data.csv} by default).
...
As the purpose of the \code{prepare_data.R} file is to prepare data for sharing,
the final line of this file
should typically be either
\code{open_data()} or \code{closed_data()}. 

After generating tidy, shareable data, this data is then loaded in the analysis code with the function \code{load_data()}.
...
the original data.
They can simply start the script with \code{load_data()}, and
write their code based on the synthetic data.
Then, they can submit their code
to you - by email or as a
["Pull request"](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request).
When you run their code on your system, \code{load_data()} will load the
...
As of version 0.1.2, the \pkg{worcs} package only stores data as \code{.csv} files.
As explained above, this is recommended, because \code{.csv} files are human- and
...
The \code{prepare_data.R} file should document any
...
development of the \pkg{repro} package [@repro], which will offer greater support for
...
For instance, \proglang{R} users can use the package \code{simstudy} [@goldfeldSimstudySimulationStudy2020].
...
The \pkg{worcs} workflow facilitates preregistration,
primarily by explicitly inviting a user to write the preregistration document.
To this end, \pkg{worcs} imports several preregistration templates from the \proglang{R} package
\code{prereg} [@austPreregMarkdownTemplates2019], including templates from
...
When initializing an RStudio project with the \pkg{worcs} project template, one of
these preregistration templates can be selected, which will generate a file
called \code{preregistration.Rmd}. This file should be used to document study plans,
...
The \pkg{worcs} package additionally
facilitates meeting the FAIR principles in several ways. First, each \pkg{worcs}
project has a YAML file (https://yaml.org/) that lists the research objects in
the project, which will allow web crawlers 
to index \pkg{worcs} projects. Second, a codebook is automatically generated for
each data file stored
using \code{open_data()} or \code{closed_data()}. We encourage users to elaborate on the
...
Incidentally, \pkg{worcs} itself follows the recommendations for [FAIR research software](https://fair-software.eu/). It is 
...
enables the citation of the software (using the \code{citation("worcs")} command, or
...
The workflow is supported by an \proglang{R} package with an RStudio project template and 
...
\pkg{worcs} package can receive a badge, displayed on the \code{README.md} file,
based on a semi-automated scoring of the checklist items, by calling the function
\code{check_worcs()} within a \pkg{worcs} project directory.
...
can be conducted entirely within \proglang{R}, resulting in a much more lightweight
...
curve associated with the tools outlined in this paper. Learning to work with \proglang{R},
...
tends to pay off. Working with \proglang{R} opens the door to many cutting edge analysis
...
the manuscript to Word (.docx), by changing the line \code{output: papaja::apa6_pdf}
in the manuscript's YAML header to \code{output: papaja::apa6_docx}. There are some
...
changes into the \code{manuscript.Rmd} file. In most cases, this is the most
...
case, send collaborators the \code{manuscript.Rmd} file, and ask them to open (and
...
@peikertReproducibleDataAnalysis2019 mention the [\code{redoc} package](https://noamross.github.io/redoc/articles/mixed-workflows-with-redoc.html) [@rossRedocReversibleReproducible2020] as a
potentially useful way to collaborate with Word users. This package enables
bidirectional conversion between Word and
RMarkdown, with support for tracked changes. However, \code{redoc} is currently in
...
paper became a co-author on several \proglang{R} packages after submitting pull requests
...
paper became involved by contributing pull requests to \pkg{worcs}. It is also
...
Firstly, although the workflow is currently implemented only in \proglang{R}, it is 
conceptually relevant for users of other statistical programming languages, most
notably \proglang{Python} [@10.5555/1593511].
We welcome efforts to implement WORCS in other platforms.
Secondly, even when a project is in \proglang{R}, it may have dependencies outside of the
\proglang{R} environment that cannot be managed using \pkg{renv}.
It is beyond the scope of the \pkg{worcs} package to support the incorporation of
tools outside of \proglang{R}, or to containerize a project so that it can
...
\pkg{repro}, an \proglang{R} package that will facilitate setting up reproducibility tools such
as Make and Docker.
...
environment generated using \pkg{repro}. 
This will combine the strengths of WORCS with those of the workflow proposed
...
WORCS offers a workflow for open reproducible code in science. The step-by-step procedure outlined in this tutorial helps researchers make an entire research project Open and Reproducible. The accompanying \proglang{R} package provides user-friendly support functions for several steps in the workflow, and an RStudio project template to get the project set up correctly.

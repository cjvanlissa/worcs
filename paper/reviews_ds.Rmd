...
However, 100MB is very large for tabular data; for example, if only integer numbers are stored that is $(100MB*1024^2)/(2^2) = 26214400$ unique values.

GitHub is, however, ideal version controlling analysis code, RMarkdown documents, and small to medium-sized data files.
Larger files can be linked to git using [git-annex](https://git-annex.branchable.com),
[Git Large File Storage](https://git-lfs.github.com/), and similar solutions. 
We now mention git-annex as a possible route to handle larger data files (TODO).

We agree that it is important to make open data findable beyond just providing it freely.
...
Our prefered route is to use github web hooks to link Zenodo and a WORCS-compatible repository. With this technique, Zenodo issues DOI for content hosted on github repositories including some nice extras, such as automatic badge DOI badges for the repository.
...
Response: It does not. It seems to us that this is outside of the scope of the present paper.
If a compelling case could be made by the Reviewer to include such a service,
we are open to the suggestion - but we would need to take on additional coauthors with the required expertise.
...
The Reviewer's comment does illustrate the importance of clarifying this point in the manuscript as well
To address this comment, the revised manuscript now reads:

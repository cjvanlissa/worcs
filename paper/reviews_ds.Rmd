...
We agree that GitHub is not an ideal platform for handling large data. However, it is ideal for handling version control of computer code, Markdown documents, and small to medium-sized data files. Larger files can be linked to git using git-annex (https://git-annex.branchable.com) and similar solutions. We now mention git-annex as a possible route to handle larger data files (TODO). We agree that it is important to make open data findable beyond just providing it freely. Digital object identifiers are the de-factor standard for making unique references to data sets. In line with the reviewer's suggestion, we suggest to use external providers to provide such DOIs while github is still the primary data storage. Our prefered route is to use github web hooks to link Zenodo and a WORCS-compatible repository. With this technique, Zenodo issues DOI for content hosted on github repositories including some nice extras, such as automatic badge DOI badges for the repository.

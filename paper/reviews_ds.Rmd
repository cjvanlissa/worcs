...
However, we acknowledge that this requires further clarification.


Particularly the addition of a robust citation for the data is an essential step that should have been mentioned in the manuscript.
In the revision, we have addressed this by adding a step to the WORCS workflow, and discussing DOIs in the paper.

Unfortunately, the Reviewer preference for Zenodo or FigShare is not explained.
We consider these platforms to be somewhat interchangeable with using Git remote repositories - with one key distinction:
A crucial advantage of storing the data on GitHub is that they will be bundled with the data documentation (codebook), analysis code, and manuscript.
Thus, readers have access to **all** raw materials in one repository.
Moreover, GitHub additionally offers user interaction with the materials through
forking, cloning, opening issues, and sending pull requests.
Thus, this platform is more conducive to collaboration.

...
To address the Reviewer's comment, we have set up an evaluation questionnaire, and sent it to the attendees of our previous workshops on WORCS. At the time of writing, however, this has not yielded actionable points of improvement.

We do have a strong history of addressing user comments submitted through the GitHub issues page.
...
It is also evident from the [resolved tickets on GitHub](https://github.com/cjvanlissa/worcs/issues?q=is%3Aissue+is%3Aclosed) that we have been very responsive in actively incorporating user feedback - as also required by the CII Best Practices.

We are committed to continue evaluating user experience, both actively through evaluation forms, and passively through the GitHub issues pages.
We hope that this commitment, along with the evidence of our prior responsiveness to user feedback presented above, assuage the Reviewer's concerns.

Finally, we wish to point out that the Editor has indicated that 'resource papers do not need to have a full-blown user evaluation, but we expect "sound evidence of its (potential for) reuse"'. We therefore elaborate on the efforts we have made to ensure that WORCS is reusable, followed by evidence of its current adoption rates.
...
1. [The preprint](https://psyarxiv.com/k4wde/) has been downloaded 568 times, since being published on 31-05-2020
1. The `worcs` R-package has been downloaded 2474 times from CRAN, since being published on 18-05-2020
...
We do take the suggestion of performing a user evaluation very seriously, but in our mind, this is a next step. We have built WORCS based on established tools and best practices, and we have gathered preliminary positive feedback from the community through workshops and teaching. 
...
After making these definitions explicit, it becomes clear that the Reviewer's definition of replication as "a necessary steps towards reproducibility" is not consistent with the definitions of the present paper.
...
\RC{C7b: Second, it doesn't follow very well the FAIR principles for findability and attribution of data. Instead, repositories like Zenodo or FigShare could be user to address data storage, with proper metadata and its corresponding citation.}

We agree completely that it is important to make open data findable beyond just providing it freely.
This is achieved by linking the GitHub repository to an OSF.io project page;
on OSF, the project receives a DOI.
It is also possible to give a separate DOI to specific resources, such as data files.
We agree that this was not emphasized enough in the manuscript, and have rewritten the revision to clarify.

It is a pity that the Reviewer does not provide arguments to support a preference for Zenodo or FigShare over GitHub, because if there is a strong case to be made, we would be keen to include this in the paper.
Without such arguments, as far as we are concerned, these repositories are largely interchangeable.

We prefer storing the data on GitHub because they are then bundled with the
...
This is redundant with Step 15 of the workflow, however:
Connecting the GitHub repository to an OSF project accomplishes effectively the same.
We do acknowledge, however, 
that this part of the workflow was not clearly documented in the previous version of the manuscript, 
and that the use of DOIs warrants further clarification.

In the revision, we have attempted to address this comment in two ways: First, by
clarifying Step 15 in the WORCS workflow, specifically emphasizing the importance of generating a DOI. 
Second, by discussing the use of DOIs in the paper.

The approach we advocate is to make all of a project's resources available in a 
Step 15 of the workflow, "Connect your ‘OSF’ project page to the ‘Git’ remote repository",
was intended to meet this goal.


Digital object identifiers are the de-factor standard for making unique references to data sets. In line with the reviewer's suggestion,
we suggest to use external providers to provide such DOIs while github is still the primary data storage. TODO!!!
...
AB: "on second thought, it’s probably worth mentioning to the reviewer that we know these options but defend the OSF-centric approach for our purposes. My impression is that the main challenge are the different expectations and solutions across disciplines that are currently evolving largely in parallel. If we acknowledge that there are alternatives, this may please the reviewers while we still can defend R + OSF for the behavioral and social sciences. Other disciplines may have different foci, e.g., python + jupyter + docker etc... and no single manuscript will be able to cover everything."
...
Response: 
F: Dit heeft weer met DOIs te maken. Ik denk dat als we het gebruik van OSF de default van worcs maken (die mijn opmerking R1C7a) dat we hier een sterk punt kunnen maken mbt findability. 
A: Eigenlijk hetzelfde antwoord als bij Findability: dit wordt heel eenvouding als we de OSF route voor data storage de default maken. 
I: Het feit dat we alleen gebruik maken van open-source software maakt alle projecten die worcs gebruiken nogal interoperable. 
R: Dit is wel een belangrijkt punt: Kunnen we opnemen in de workflow dat worcs-projecten by default een CC-BY licentie krijgen?
...
Of course, since `worcs` is open source software, anybody is welcome to contribute this functionality in exchange for coauthorship, in line with the contribution guidelines [outlined here](https://github.com/cjvanlissa/worcs#contributing-and-contact-information).
The current team lacks relevant expertise to make this contribution.
...
> Git tracks changes to files on a line-by-line basis. The user can sture these changes by making a "commit": a 
...
We agree that this is an important point, and have rewritten the paragraph comparing these two solutions (as cited below). We hope that this adresses the Reviewer's question. However, as the workflow of Peikert and Brandmaier is still in press, and the software implementation has not yet been fully developed - a more detailed comparison of these two approaches would be premature.

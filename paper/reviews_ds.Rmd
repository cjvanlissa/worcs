...
Second, there is some evidence to demonstrate that worcs is already being adopted:
1) The worcs R-package has been downloaded 2255 times from CRAN (GitHub downloads are not tracked)
2) The video tutorial for worcs has been viewed 356 times, and liked 18 times (https://www.youtube.com/watch?v=uzjpN_yFeUU)
3) Worcs is the software used for reproducibility education at Utrecht University
4) The lead author has given invited lectures on worcs at the Erasmus University Rotterdam, Eindhoven University of Technology, and Max Planck Institute, Berlin
5) The lead author has applied worcs in several projects:
...
Response: Respectfully, we are not entirely sure what the criticism is here. First, there seems to be a misunderstanding about the scope of the paper, which is described in the Abstract: *"This paper introduces [a] step-by-step procedure that researchers can follow to make a research project open and reproducible, accompanied by an open source software implementation that automates these steps"*.

Our paper thus introduces a workflow for open science, based on objective criteria set out in the TOP-guidelines. Aside from the work in progress of our co-authors - which is focused on strict computational reproducibility and is compatible with the present workflow - this is one of the first efforts to translate the requirements of open science into a step-by-step workflow. When you search Google Scholar for "open science workflow", the first hit that actually describes a workflow is the present manuscript.

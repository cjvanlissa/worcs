...
author:
  ...
  affiliation: | 
      ...
      g: Humboldt-Universitaet zu Berlin  
      h: Utrecht University faculty of Humanities, department of Language, Literature & Communication, Utrecht Institute of Linguistics OTS  
      i: University Library Utrecht
...
abstract: |
  ...
  standards for open science, and provides examples of the implementation in `R`. 
  This paper is primarily targeted towards
  scholars conducting research projects in R on a private computer,
  ...
  and a list of user examples of WORCS projects, are available at 
...
This paper is most relevant for scholars using R on a private computer to
conducting research that involves academic prose, analysis code,
and tabular data.
...
In this definition, the term "science" refers to all scholarly disciplines.
...
The user can sture these changes by making a "commit": a 
...
platform 'Code Ocean'.
Although Code Ocean enjoys some popularity among 
...
data are stored.
The necessity to upload human participant data for analysis is an important consideration and potential liability when using any cloud-computing solution.
...
For an up-to-date list of sample `worcs` projects, see the [WORCS GitHub page](https://github.com/cjvanlissa/worcs).
...
synthesis (see @wrightRangerFastImplementation2015).
...
Second, WORCS is easier to use, as it only requires familiarity with R.
WORCS was designed to tightly integrate into the R ecosystem, bringing down the learning curve by utilizing the users' knowledge of R.
...
as Peikert and Brandmaier, and the two workflows are compatible. What sets WORCS apart is that it is more lightweight in terms of system and user requirements, but still ensures computational reproducibility under *most circumstances* for projects that can be conducted entirely within `R`.

...
abstract: |
  ...
  `worcs` in `R`. The source code for the `R` package and manuscript,
...
For analyses conducted entirely in `R`, the recently released package `renv` [@usheyRenvProjectEnvironments2020]
strikes a great balance between user-friendliness and robustness.
Developed by the team behind RStudio, 
this package simply maintains a text-based,
...
The text-based log file can be version controlled with Git.
The `renv` function `snapshot()` is used to add packages to the log file.
This function identifies all packages loaded using `library()` calls in the scripts in the project directory.
...
When someone else loads your project, `renv` will install all of the required packages.
As opposed to alternative solutions, `renv` is tightly integrated with the
native R package management system.
Therefore, it does not require the users to change their workflow of installing/removing and using packages, and requires no technical skill to setup.
...
If stricter reproducibility is required,
users might choose to conduct all analyses in a "Docker container": An environment that behaves like a
...
describes how to instantiate a Docker build running `worcs` and its dependencies.
...
To overcome this challenge, an `R` package is currently in development to facilitate Docker integration [`repro`, @repro].
Another 
...
Several arguments support the choice to implement this workflow in `R` and RStudio first, although we encourage developers to port the software to other languages.

...
<!--
Barbara:
A trick I learned in collaborative writing with git is to make sure each new sentence is on a new line.
This way, you can effectively use a git diff to see changes in individual sentences instead of having to scour through an entire paragraph.
I'll add this, but only in the paragraphs I've changed, so you do not get inundated with differences!
-->
...
The TOP-guidelines are primarily geared towards top-down systemic change by journals, funders, and organizations.
However, large organizations are slow to change.
For example, a snapshot of editorial practices indicated that, in 2015, only 3% out of 1151 psychology journals explicitly stated that they accepted replication studies [@martinArePsychologyJournals2017; see also @klebelPeerReviewPreprint2020].
<!-- A more recent preprint reported that many journals do not explicitly state their policies about  klebelPeerReviewPreprint2020 -->
...
The formation of "Open Science Communities" at universities around the globe illustrates this grassroots movement.
...
This workflow can be used regardless of any top-down open science requirements already implemented by journals and institutions, and where such requirements do exist, this workflow will help fulfill them.
...
## Prerequisites
<!--Barbara: I'm moving up this section and editing it a bit, to introduce more explicitly that worcs was developed for R.-->
WORCS has focused on developing a workflow for R, instead of for SPSS or Python.
<!-- Barbara: same as for Git, I could not find a citation. Maybe cite the website: www.r-project.org -->
...
Second, many tools for an open science workflow are already implemented in R.
A vibrant online community exists for R, developing new R methods and packages, and providing support and tutorials for existing ones.
Wrappers in R are often available for tools developed in other programming languages.
<!-- LB: Perhaps give some pointers to learn R (e.g. R4DataScience)-->
Third, at the time of writing, R is the second-most cited statistical software package [@muenchenPopularityDataScience2012], following SPSS, which has no support for any of the open science tools discussed in this paper.
Finally, R is highly interoperable:
Packages are available to load nearly every imaginable filetype, code written in other programming languages [including Python, @allaireMarkdownPythonEngine] can be called from R, and output can be written to many file types, including DOCX and PDF (in APA style), and HTML format. 

Working with R is simplified immensely by using the RStudio integrated development engine (IDE) for R.
<!-- Barbara: 
https://support.rstudio.com/hc/en-us/articles/206212048-Citing-RStudio says:
To cite RStudio in publications, you can get the latest citation information by running the command RStudio.Version() in a recent version of RStudio IDE.

I did this, which gave:
...
-->
Many of the tools used in WORCS are embedded directly into the user interface of RStudio.
RStudio automates and streamlines many tedious or complicated aspects of working with R, Rmarkdown, and Git/GitHub.
...
When a new project is initialized from this template, much of the bookkeeping required to set up an open science project is performed automatically.
...
Maintaining an indelible log of every change to all project files. Version control is a near-essential tool for scientific reproducibility, as anyone learns who has had the experience of accidentally deleting a crucial file, or of being unable to reproduce analyses because they ran some analyses interactively instead of using syntax [see also @blischakQuickIntroductionVersion2016].
Many scientists use some form of *implicit* version control; for example, by renaming files after major changes (e.g., "manuscript_final_2.2-2019-10-12.doc"), tracking changes in word processing software, or using cloud hosting services that retain backups of previous versions.
WORCS instead uses the *explicit* version control software "Git". Git tracks changes to files, and stores these changes when the user makes a "commit".
<!--Barbara: I have looked for a way to cite Git but did not find one... Seems good practice especially in this paper as an example to cite all software, but for Git I just cannot find the way. Maybe cite the website: www.git-scm.com -->
Git retains a complete log of all commits, and users can compare changes between different commits, or go back to a previous version of the code (for example, after making a mistake, or to replicate a previous version of the results).
A project version controlled with Git is called a "Repository", or Repo.
It is also possible to prevent some files from being version controlled - such as privacy sensitive data.
The `.gitignore` file is a list of files that should not be version-controlled, and thus exist only on the user's private computer.
...
This is important because most preprint servers - [at least the ones listed by OSF.io](https://help.osf.io/hc/en-us/articles/360019930493-Preprint-FAQs#how_do_i_find_preprints) - are indexed by Google Scholar.
...
<!-- Barbara: I see no difference between these two symbols...-->
...
When data cannot be shared, researchers should aim to safeguard the potential for computational reproducibility and secondary analysis as much as possible.
WORCS uses two solutions to accomplish this goal.
The first solution is publishing a *checksum* of the raw data upon collection.
A checksum is the result of an algorithm applied to a file, and represents the contents of this file in 32-characters.
Any change to the underlying file results in a different checksum. Thus, one can use a checksum to verify that the contents of a file remain unchanged.
If researchers publish the checksum of their raw data, in addition to the complete analysis code, then they can prove to an independent auditer that running the public analysis code on the private dataset with the correct checksum results in the published results of their work.
...
<!--Barbara: I'm not sure this is the case. Pull requests are useful if someone wants to change or add to a project, but this would not involve a change to an existing project, but rather a new project/analysis with "old" data. I would remove this suggestion altogether.-->
...
<!--Barbara: Is it indeed the case that data by default is loaded as a .csv? How about (raw) text data; can worcs accommodate a .txt file, for instance (in case of text analysis)? Or should this all be done in prepare_data.R?-->

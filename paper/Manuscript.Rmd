...
Sharing all research output also provides a proper basis for research evaluation according to the San Francisco Declaration on Research Assessment (DORA; https://sfdora.org/), which increasingly plays a role in grant funding and hiring or promotion procedures. Direct access to research objects allows funding bodies and hiring committies to evaluate research quality based on content rather than relying on spurious surrogate indicators like journal impact factors, conference rankings, and h-indexes. The detailed version control and commit tracking of github and similar platforms furthermore make it possible to assess the relative contributions made by different researchers. 

The FAIR Guiding Principles for scientific data management and stewardship [@wilkinsonFAIRGuidingPrinciples2016a] advocate that research data should be 
Findable, Accessible, Interoperable and Reusable. Moreover, these principles are
increasingly applied to other types of research output 
[@lamprechtFAIRPrinciplesResearch2019]. Research
organizations and funders use the FAIR principles as standard for open, 
transparent and reproducible science.
The WORCS workflow helps users meet the FAIR principles by making all research objects -
including (origina) data and code - accessible in a (public) repository. 
The `worcs` package additionally
facilitates meeting the FAIR principles in several ways. First, each `worcs` project has a YAML file
(https://yaml.org/) that lists the research objects in the project, which will allow web crawlers 
to index `worcs` projects. Second, a codebook is automatically generated for each data file stored
using `open_data()` or `closed_data()`. We encourage users to elaborate on the default codebook by adding
variable description and categories, which might - in the future - enable indexing data by topic area. 
Relatedly, `worcs` is itself an example of FAIR research software [see @lamprechtFAIRPrinciplesResearch2019 and https://fair-software.eu/]. It is hosted on a public version controlled repository ('GitHub'), it has an open source licence (GPL v3.0), is registered in a community registry (CRAN), it enables the citation of the software (using the `citation("worcs")` command, or by citing this paper), and followed the CII Best Practices software quality checklist during development.

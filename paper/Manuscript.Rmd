---
title             :  "WORCS: A Workflow for Open Reproducible Code in Science \\newline `r paste0('Version: ', worcs:::cran_version(), ', date: ', format(Sys.Date(), '%d %b,\ %Y'))`"
...
abstract: |
  ...
  goal of this paper is to remove these obstacles. We introduce the _Workflow for
  Open Reproducible Code in Science_ (WORCS). WORCS constitutes a step-by-step
  procedure that researchers can follow to make an entire project open and
  reproducible. It is based on current best practices, and can be used either in
  ...
  RStudio project template that is, effectively, a "one-click solution" for R users. 
...
authornote: |
  ...
  VI.Veni.191G.090).
  We acknowledge Jeroen Ooms for offering feedback and suggesting the `gert` package.
...
crisis. As the late Jonathan Tennant brilliantly put it: "Open science is just good science"
...
that meets most current requirements of open science, and is based on recent best practices: The _Workflow for Open Reproducible Code in Science_ (WORCS).
...
research project (including data, code, and a report) open and reproducible. WORCS is compatible with open science requirements already implemented by journals and institutions, and will help to
fulfill them. It can also be used in the absence of top-down support or guidelines
to produce work in accordance with current best practices.
...
[for a proposition to arrive at a unified notion, see 
@tennantValuePropositionOpen2020].<!--AB: I wasn't sure what was meant with 'first principles'; I think it's more about the unification-->
Instead, there are several guidelines for practices that are considered "open".
The TOP-guidelines are one of the most influential operationalisations <!--AB: operationalisations are always concrete-->
...
the subject of a research project, not to its organisation or execution.
...
Although there is a slight learning curve to transitioning to DDG, we strongly believe that the
investment really pays off<!--AB: It's a claim after all, so let's say "we strongly believe"-->. Time saved from painstakingly copy-pasting output
...
function or using a special syntax for calling labeled chunks of code. When a reader or reviewer compiles this document, all code is run
...
The second tool <!--AB: when saying solution, we need a problem--> is *version control*:
...
'GitLab' (https://gitlab.com) and 'Bitbucket' (https://bitbucket.org) <!--AB: https://gitea.io/ which is a clone of Gogs seems also pretty cool-->. As of
...
The third tool <!--AB: Same as before--> is *dependency management*: Keeping track of exactly what
...
Peikert and Brandmaier can be difficult to set up for novice users. The accompanying R package `repro`
...
Second, as of this writing, R is among the <!--AB: I think python folks will not be happy about this comment if we say R is most advanced--> most advanced programming language in terms
...
Fourth, R is well-supported; a vibrant and inclusive online community exists for R, developing new R methods and packages, and providing support and tutorials for existing ones.
...
[DORA](https://sfdora.org/). <!--AB: spell out-->
...
Any change^[It is possible that random changes to a file result in the same checksum but chances are low] <!--AB: I would really urge us to be precise about this. If checksums were really unique, we would have a really awesome compression algorithm but by applying the pigeonhole principle, it is easy to show that many large files must map onto the same short checksum--> to the file will result in a different checksum. 
...
corpora can have one row per document, and EEG- or ECG waveforms can have one
row per channel. The `prepare_data.R` file should document any
...
<!--AB: the 'excellent' seems preposterous now that we joined this project--> workflow strives to ensure strict computational reproducibility for

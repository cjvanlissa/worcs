...
abstract: |
  ...
  potential to accelerate scientific progress. However, adopting open science
  principles can be challenging and costly, because doing so requires
  ...
  institutions, and funding bodies. To further facilitate the transition, the
  WORCS principles are implemented in an R-package, which offers an RStudio 
...
Moreover, open science creates opportunities for researchers to more easily
...
adopt open science practices can be daunting. Making the transition
...
that meets most requirements open science, is based on best practices, and 
is supported by user-friendly software in R: The Worfkflow for Open Reproducible 
...
[but, for an effort to ground open science in first principles, see 
...
Fortran, and Python [@allaireMarkdownPythonEngine]. There are excellent free
...
several potential styles. When the text is formatted, the results of the code
blocks are
...
[see @allaireRticlesArticleFormats2020], and as plain 'Markdown' for 'GitHub'.
...
WORCS recommends using the *explicit* version control software 'Git'
...
[@oomsGertSimpleGit2019], to connect a local project to a remote repository on
'GitHub'. The `worcs` package also contains a user-friendly function to set
the default user credentials, `git_user()`, and a function to add, commit, and
push all changed files in one step, `git_update()`. It is worth noting that,
although this paper focuses on 'GitHub', which is the most widely used 'Git'
cloud storage service, two notable alternative services exist: 
'GitLab' (https://gitlab.com) and 'Bitbucket' (https://bitbucket.org). As of
version 0.1.2, the `worcs` template dialog accepts 'https' addresses of 
repositories on both of these platforms interchangeably. The package has a
[vignette](https://cjvanlissa.github.io/worcs/articles/git_cloud.html) with
additional instructions for using these platforms. However,
`worcs` has been most extensively tested with 'GitHub'.
...
hosting services, such as 'GitHub'. For these reasons, uploading large binary
files to these platforms is frowned upon.
'GitHub' encourages the use of text-based files by allowing visitors of their website to peruse
text-based files online, and
...
`worcs` uses text-based file types like Markdown and `.csv` extensively.
...
After setting up your system, you can use the WORCS workflow for all of your
projects. This step-by-step procedure is detailed in [the workflow vignette](https://cjvanlissa.github.io/worcs/articles/workflow.html).
In this paper, we provide only a brief outline of the WORCS procedure (as of Version `r worcs:::cran_version()`) below.
We highly recommend users to refer to [the workflow vignette](https://cjvanlissa.github.io/worcs/articles/setup.html) for the most up-to-date procedure (subject to improvements).
...
in the source code repository (e.g., on 'GitHub'), using the
...
all teps necessary to load the 
...
pushed to GitHub. Once the GitHub repository is made public, these data will be
open to the public.
...
The user should push all changed files 'GitHub'.
Once the 'GitHub' repository is made public, people will have access to a
...
The analysis code, on the other hand, should begin with `load_data()`.
...
users who copy a 'GitHub' repository with closed data will automatically load the synthetic
...
on GitHub. When you run their code on your system, `load_data()` will detect the
original data, and use that to run their script. You can then return the results
to the third party.

As of version `r worcs:::cran_version`, the `worcs` package only stores data as `.csv` files.
...
stored in a different file type anyway, readers are encouraged to follow the
...
to the 'Git' repository, and making this repository Public, e.g., on 'GitHub'.
...
with the study plans to, e.g., 'GitHub'.
...
documenting the planned analyses.
...
e.g., through pull requests on 'GitHub'. If the repository is Private, Reviewers
can be invited as "Collaborators". Private repositories can be made public at a

...
practices in the absence of top-down support. The WORCS workflow is supported by
an R-package with user-friendly support functions, and an RStudio project
template that is, effectively, a "one click solution". This paper introduces
the WORCS workflow, discusses how it meets best practices for open science, and
illustrates the use of the `worcs` package.
...
# Meeting the requirements of Open Science
...
data should be Findable, Accessible, Interoperable and Reusable. However, they are
increasingly applied as a standard for 
to other types of research output as well
[@lamprechtFAIRPrinciplesResearch2019].
WORCS helps users meet the FAIR principles by making all research objects -
including (origina) data and code - accessible in a (public) repository. 
...
Incidentally, `worcs` is itself an example of FAIR research software [see @lamprechtFAIRPrinciplesResearch2019 and https://fair-software.eu/]. It is hosted on a public version controlled repository ('GitHub'), it has an open source licence (GPL v3.0), is registered in a community registry (CRAN), it enables the citation of the software (using the `citation("worcs")` command, or by citing this paper), and followed the CII Best Practices software quality checklist during development.

Sharing all research output also provides a proper basis for research evaluation
...
designed for R [@rcoreteamLanguageEnvironmentStatistical2020].
...
Before you can use the WORCS workflow, you have to install the required software. You have to execute this setup procedure only once for each computer you intend to use `worcs` on. The procedure should take about 30 minutes, and it is documented in [the setup vignette](https://cjvanlissa.github.io/worcs/articles/setup.html). After preparing your system, you can use the WORCS workflow for all of your projects. This step-by-step procedure is detailed in [the workflow vignette](https://cjvanlissa.github.io/worcs/articles/workflow.html). In this paper, we provide only a brief outline of the WORCS procedure (as of Version `r
tryCatch({worcs:::cran_version()}, error = function(err) 'NA' )`) below. We highly recommend users to refer to [the workflow vignette](https://cjvanlissa.github.io/worcs/articles/setup.html) for the most up-to-date procedure (subject to improvements).

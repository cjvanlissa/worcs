...
abstract: |
  ...
  goal of this paper is to lower the threshold to adoption. We introduce the Workflow for
  ...
  institutions, and funding bodies. This workflow is primarily targeted towards
...
The R package `worcs` [@worcspackage2020] offers a project template for RStudio
...
stored in `.R` files, and included in the main document using the [`read_chunk()`
...
paper.
As of
version 0.1.2, `worcs` has been tested with these three remote repository services,
...
When initializing a new project using the `worcs` project template,
...
The accompanying R package `repro`
...
data are stored. The necessity to upload data for analysis is a risk with any
...
The `worcs` implementation for R makes use of this fact by, for example,
rendering a codebook for the data as `.md` so visitors of a 'Git' remote repository
...
Fourth, R is well-supported;
a vibrant and inclusive online community exists for R,
developing new R methods and packages,
...
By [tagging the commit as a release](https://docs.github.com/en/free-pro-team@latest/github/administering-a-repository/managing-releases-in-a-repository), labelled "preregistration"
...
Incidentally, `worcs` itself follows the recommendations for FAIR research software [see https://fair-software.eu/]. It is 
...
<!-- Somehow this feels like very sudden and incomplete ending to me. I am sure you can add some more glorious last words. :) --> 

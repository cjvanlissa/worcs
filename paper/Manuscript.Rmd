...
author:
  ...
  - name: "Marijn E. Struiksma"
    affiliation: "2,9"
    email: "m.struiksma@uu.nl"

<!-- I would suggest to put OSCU as last item in the list of affiliations, as it is complementary to the primary institutional affiliations --> 
affiliation:
  - id: "1"
  ...
  - id: "9"
    institution: "Utrecht University, Department of Language, Literature & Communication, Utrecht Institute of Linguistics OTS"
...
abstract: |
  ...
  procedure that researchers can follow to make an entire <!-- Here I would include a word for describing the kind of project. "data science" maybe? --> project open and
...
authornote: |
  ...
  VI.Veni.191G.090).
  We acknowledge Jeroen Ooms for offering feedback and suggesting gert.
keywords          : "open science; reproducibility; r; dynamic document generation; version control, FAIR"
wordcount         : "8257"
...
The (social) sciences <!-- Is the "(social)" necessary here? Does it add anything? How about just "science", or alternatively "research" or "academia"? --> are arguably past the tipping point of a paradigm shift
...
crisis. As the late dr. <!-- Is it common to include the title? Maybe rather a description of who he was, his function? --> Jonathan Tennant put it: "Open science is just good science"
...
The Workflow for Open Reproducible 
Code in Science (WORCS). WORCS is a lightweight approach to open science and
computational reproducibility <!-- here another good point to say something about the scope -->. The workflow (Figure <!--\@ref(fig:workflow)-->) 
...
<!-- maybe give a short outline of this section here --> 
...
website or blog post. Moreover, reproducibility is guaranteed <!-- that is a strong claim, and probably not true. For example, if the code calls a remote service, that might change its interface or discontinue operation, and the code will not be reproducible anymore - so, suggestion is to weeken this claim a bit... -->, and can be
...
time. <!-- https://www.scientificpaperofthefuture.org/ could be referenced here --> 
...
instead of using syntax <!-- Also interactive analyses often use syntax, so this is not a good distinction.  "... instead of storing commands in a script file" or something like that would be more precise --> [see also @blischakQuickIntroductionVersion2016].
...
Text-based files are also required when using 'Git' for version control.
In 'Git', each change reflects a line of text added or removed. This means that, when
...
get_workflow()
```
<!-- Some questions on the workflow, for simplicity I put them here: 
Step 6: "a script documenting the steps" - is that an executable script, or just documentation, or a script with comments? 
Step 7: What about external data sources, actuall? For example, when I use the daily corona stats from Johns Hopkins university directly from their online source. Should they be copied or just a link provided? For FAIR (or at least F) data (with a PID) a link would suffice, but maybe for other sources a copy is advisable. 
Step 14: Is that optional, too? And could it not be done earlier, already in Phase 1?-->

## The R-implementation of WORCS

The R-package `worcs` automate most steps of the workflow
for R users.
...
of the implementation of tools required for an open science workflow. <!-- I would omit the second argument. It is highly debatable, and as it stands not more than a claim. Without having looked at the details, I would for example also see Python absolutely capable of doing this. The other four arguments are completely sufficient for motivating the focus on R, so IMHO no reason to open this discussion. --> 
...
[DORA](https://sfdora.org/). <!-- Maybe due to restructuring, but so far DORA has not been mentioned in the paper. Easiest way to solve this would be to remove the subordinate clause at the end, and leave the discussion of DORA to the respective section later. --> 
...
<!-- just today the Software Heritage Project announced https://www.softwareheritage.org/2020/05/26/citing-software-with-style/ - might also be worth mentioning here? --> 
...
This is important because most major preprint servers - at least those hosted <!-- are these "most"? not sure this remark is very accurate --> by the [Open Science Framework (OSF)](https://help.osf.io/hc/en-us/articles/360019930493-Preprint-FAQs#how_do_i_find_preprints) - are indexed by Google Scholar.
...
publicly available. This function stores the `data` object (a `data.frame` or
`matrix`) in a spreadsheet file (called `data.csv` by default), and generates an
accompanying codebook that can be viewed online.
All changed files should now be (manually) added to the 'Git' repository, committed, and
...
format as close to "raw data" as possible, the final line of this file
should typically be either
`open_data()` or `closed_data()`. 
The analysis code, on the other hand, should begin with `load_data()`. <!-- Is this actually limited to one data file, or can it handle multiple ones? --> 
This function loads the real data if it is available on the user's computer, and
...
the original data. They can simply start the script with `load_data()`, and
write their code based on the synthetic data. Then, they can submit their code
to you - by email or as a
["Pull request"](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request).
When you run their code on your system, `load_data()` will detect the
...
machine readable, and because data typically need to be in a spreadsheet format
for analysis. Many types of data can be represented as a spreadsheet - text
...
steps required to convert these data into tabular format. If it is data must be
stored in a different file type anyway <!-- something is wrong with the (beginning of) this sentence -->, readers are encouraged to follow the
...
However, 'Git' cloud hosting services also conform to these standards. <!-- I am not familiar with these standard, but AFAIK the hosting services make no guarantees regarding long-term availablity of the repositories, in particular not about stable URLs pointing to repositories. They will not disappear suddenly or in the next few years, probably, but if it is important for preregistrations to be available "forever", then they should be made at a service that provides a permanent storage and identifier.  -->
Thus, it is possible to preregister a study simply by pushing a text document
with the study plans to a 'Git' remote repository.
By tagging the commit as a preregistration
(see [the workflow vignette](https://cjvanlissa.github.io/worcs/articles/setup.html)),
it is distinct from all other commits, and easily findable by people and programs.
...
discussion of the topic]. Preregistration is the only way to ensure that a
...
In all cases, researchers should minimize exposure to the data, and disclose any
exposure.
...
to handle unforseen contingencies, such as violations of model assumptions;
...
To preregister a project using 'Git', one can commit
all preregistration documents to the local 'Git' repository, 
and push them 
to a remote 'Git' hosting service.
By [taging the release](https://help.github.com/en/github/administering-a-repository/creating-releases) as "Preregistration" on the remote repository, it can be easily identified as
such and retrieved. This simple but robust approach renders pre-registration through
...
by citing this paper), and followed the CII Best Practices <!-- reference? --> software quality
...
WORCS has its own checklist, <!-- incomplete sentence! -->
...
excellent <!-- I am not opposing this statement, but given that the two authors of this work are now co-authors on this very paper, it is a bit awkward to use this qualification... ;) --> workflow strives to ensure strict computational reproducibility for
...
as Peikert and Brandmaier <!-- just noting that their names are frequently repeated. has their approach/tool/... a name that could be used instead? -->, which means the two workflows are compatible. What 
...
TOP-guidelines <!-- was it not with one exception? then maybe just replace the "all" in this sentence by "the" -->. Nevertheless, there are clear directions for future developments.

...
archiving a `worcs` project on ‘Zenodo’. As open science infrastructure
is an area of active development, the approach proposed here should not
be considered definitive, but rather, as one proposal for a
FAIR-compliant way to archive code repositories using infrastructure
...
Make sure that the local repository is connected to a remote (‘GitHub’)
repository
...
We will use ‘Git’ to version control the project folder. If you do not
...
If ‘Git’ is indeed installed, you can check whether your current project
...
You should receive a notification that
`"Active project has a remote repository"`. If you do not receive this
notification, first connect your local repository to GitHub.
...
‘GitHub’ website. Either way, replace `repository_name` with the actual
...
To ensure that your local R-package dependencies are correctly archived,
your project should use `renv`. Before actually archiving the project,
you should make sure that these dependencies are up to date. Do this by
running:
...
The `worcs` function
[`worcs::git_update()`](https://cjvanlissa.github.io/worcs/reference/git_update.md)
...
files are now visible in the remote repository. Furthermore, the
repository visibility must be set to “Public” to ensure that ‘Zenodo’
can discover and archive it. If necessary, change the visibility setting
to Public by clicking on “Settings” \> “General” \> “Change repository
visibility.”
...
major change to your project, you may want to manually increment the
...
We can further document our ‘Zenodo’ archive by adding some extra
information on ‘Zenodo’. On ‘Zenodo’ click the
...
citations or references to your code to enhance its discoverability and
reusability.
...
unique DOI to each version/release of the code. This enables users to
refer to and cite either the repository in general or specific versions
of it. The list of authors for the citation is automatically determined
by the ‘GitHub’ user account names used by the repository - this can be
edited on ‘Zenodo’, as explained above. DOIs used in ‘Zenodo’ are
registered through the [DataCite](https://www.datacite.org/) service.
...
Your repository is now archived in ‘Zenodo’, and with a DOI that can be
...
including code, and this workflow enables that to happen, in line with
best practices.

> **Pro-tip**: Is your research funded by an EU grant? Now you can
> directly connect your repository to your grant by updating the grant

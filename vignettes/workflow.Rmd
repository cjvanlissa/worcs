...
10. <!--S: Commit every small change to the 'Git' repository-->Regularly Commit your progress to the Git repository; ideally, after completing each small and clearly defined task.
    + To Add all new files (not blocked by `.GitIgnore`), Commit them, and Push the changes to a remote repository all in one step, call `git_update("Describe the changes made since the last commit")`

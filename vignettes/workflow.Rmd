...
1. <!--S: Create a new Private repository on 'GitHub'-->Create a new repository on 'GitHub'
    + For inexperienced users, we recommend making this repository "Private", which means only you and selected co-authors can access it. You can set it to "Public" later - for example, when the paper goes to print - and the entire history of the Repository will be public record. If you are an experienced user and are sure that you want your Repository to be public from the start, you can make it Public.
    + Copy the https:// link to clipboard; this link should look something like https://[...].git
...
7. <!--S: Save the data using `open_data()` or `closed_data()`. __WARNING: Never commit data to 'Git' that you do not intend to share__-->Save the data using `open_data()` or `closed_data()`
...
12. <!--S: Add an open science statement to the Abstract or Author notes, which links to the 'GitHub' repository or 'OSF' page-->Add an open science statement to the manuscript, which links to the 'GitHub' repository or 'OSF' page. 
    ...
    and a tagged commit with the preregistration._
12. <!--S: Knit the paper to PDF-->Knit the paper to PDF for submission
...
17. <!--S: Optional: Publish a preprint-->Optional: [Publish preprint in a not-for-profit preprint repository such as PsyArchiv, and connect it to your existing OSF project](https://help.osf.io/hc/en-us/articles/360019930533-Upload-a-Preprint)
...
18. <!--S: Optional: Add a WORCS-badge to your README file and complete the optional elements of the WORCS checklist-->Run the `check_worcs()` function to see whether your project adheres to the WORCS checklist (see `worcs::checklist`)
